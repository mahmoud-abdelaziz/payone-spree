...
module PayOne
  module Provider
    module Payment
      class Base < PayOne::Provider::Base
        ...
        def set_sequence_request_parameters(request, response_code)
          request.sequencenumber= PayOne::RequestHistory.count_overall_status_by_txid response_code
        end
        ...
        def set_status_url_request_parameters(request, gateway_options)
          if gateway_options[:admin_created].to_s == 'true'
            request.successurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_success_admin_order_payment_url(
            ...
            request.errorurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_error_admin_order_payment_url(
                gateway_options[:order_id],
                gateway_options[:payment_id],
                :host => Spree::Config[:payone_engine_host],
                :token => gateway_options[:order_token],
            ...
            request.backurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_back_admin_order_payment_url(
                gateway_options[:order_id],
                gateway_options[:payment_id],
                :host => Spree::Config[:payone_engine_host],
                :token => gateway_options[:order_token],
            ...
            request.successurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_success_order_checkout_url(
                gateway_options[:order_id],
                :host => Spree::Config[:payone_engine_host],
                :token => gateway_options[:order_token],
                :action_token => success_token,
            ...
            request.errorurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_error_order_checkout_url(
                gateway_options[:order_id],
                :host => Spree::Config[:payone_engine_host],
                :token => gateway_options[:order_token],
                :action_token => error_token,
            ...
            request.backurl= 
              Spree::Core::Engine.routes.url_helpers.pay_one_back_order_checkout_url(
                gateway_options[:order_id],
                :host => Spree::Config[:payone_engine_host],
                :token => gateway_options[:order_token],
                :action_token => back_token,
        ...
        def payment_provider_response(response)
          ...
          if state == :success || state == :redirect
            PayOne::Logger.info message + ': ' + options.to_s
          else
            PayOne::Logger.error message + ': ' + options.to_s
          end
          
          PayOne::Provider::Payment::Response.new(state, message, {}, options, response_messages)
        end
        ...
        def payment_payment_provider_response_not_supported(action_name)
          ...
          PayOne::Logger.error message
          PayOne::Provider::Payment::Response.new(false, message, {}, {}, {})
        end
        ...
        def payment_payment_provider_successful_response
          ...
          PayOne::Provider::Payment::Response.new(true, message, {}, options, {})
        end
      end
    end
  end
